...
```
1. Resolve . (orchestrator-patterns.md → ". Resolution").
2. Check whether .claude/temper.config exists in the current project:
   ...
   b. If it does NOT exist:
        ...
        - Copy the default template into place:
            templates/temper.config.default
            -> .claude/temper.config
        - Report: "Created .claude/temper.config from the default template."
3. Run `scripts/temper init` — scaffolds `.temper/` (gates ledger,
   overrides log, feedback-loops registry). Also idempotent.
4. Recommend the commit gate: "Run `bash scripts/hooks/install.sh`
...
```
scripts/temper evidence add --stage init --claim "<what you're proving>" \
  --cmd "<the exact command you ran>" --exit <code> --label PROVEN
scripts/temper gate init
```
